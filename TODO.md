...
- [ ] **Bar clock: AM/PM format, maybe an analogue clockface option** — `bar/bar.py`'s `_tick_clock` currently renders `time.strftime("%H:%M:%S")` (24-hour, with seconds); switch to a 12-hour `%I:%M %p`-style display, and consider a small drawn analogue clock as an alternative/addition.
- [ ] **Volume +/- stepper buttons** — `bar/bar.py`'s volume control is currently a bare `Gtk.Scale` slider calling `amixer -q sset Master <N>%` on change. Add explicit vol− / vol+ buttons for incremental steps, likely easier to hit accurately for small hands than dragging a slider precisely.
- [ ] **Small "(i)" info button, bottom-right of screen** (supersedes the standalone refresh icon idea above — the refresh action lives inside this instead of being its own icon). Opens a small info box containing: the admin portal URL (currently a placeholder — `http://dnsentry-tbd:8080`, real DNS entry/port not yet decided), a server-status indicator (green/red light + "last updated" timestamp, presumably driven by the same telemetry/poll success-or-failure Phase 9 already tracks), and the "refresh config now" action itself — lets a teacher force an immediate `/config` poll instead of waiting for Phase 9's interval, useful for troubleshooting ("did the new plugin show up yet") without a reboot.
...
- [ ] **WiFi status icon in the bar**, hover/tap reveals SSID, IP, or a clear "disconnected" state — ties into the school's WPA2-Enterprise (PEAP/TTLS) network already documented in CLAUDE.md's Network & Deployment Context. Worth deciding whether this needs a `nmcli`/`iwconfig` shell-out (matching the `amixer` pattern already used for volume) or something else, once picked up.
