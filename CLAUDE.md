...
- **Imaging/provisioning strategy — decided (v2), unverified on real hardware.** See "Provisioning / Imaging" above for `scripts/build-provisioning-usb.sh`/`.ps1` and the rebuilt `preseeds/classpad/preseed.cfg`. Unlike the reverted v1, this has been reasoned through carefully (FAT32-vs-ISO9660 filesystem detection, boot-config rewrite ordering, the two independent causes of v1's "boots to a text login" bug — `graphical.target` *and* `install.sh`'s `systemctl ... --now`/`daemon-reload` calls failing under `in-target`'s chroot — `CLASSPAD_SKIP_REBOOT`, and logged/conditional late_command cleanup) and has unit-test coverage for the bash script against a synthetic USB tree, but neither script has been run against a real Debian netinst ISO on real Rufus-written media yet — do that before trusting it for a real fleet rollout. WiFi provisioning specifically is still getting a fresh design pass; the only WiFi path in the repo right now is `scripts/wifi-configure.sh`'s env-var-driven `nmcli` call from `install.sh`.
