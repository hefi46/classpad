...
**Preseed-based USB provisioning, re-decided 2026-08-07 (v2).** A first version (`preseeds/classpad/preseed.cfg` + a USB-prep script wrapping the vendored third-party [debian-preseed-iso-generator](https://github.com/bergmann-max/debian-preseed-iso-generator)) was added 2026-08-06 and removed in full 2026-08-07, along with a since-abandoned first-boot interactive WiFi TUI — none of that ISO-generator vendoring or WiFi-TUI tooling came back; see git history around commits `6832d69`/`911da1d` for the record of what existed and why it was pulled. **This v2 is a different, simpler design, not a restoration of the old one:**
- **No ISO rebuild, no vendored generator.** `scripts/build-provisioning-usb.sh` (Linux/bash) and `scripts/build-provisioning-usb.ps1` (Windows/PowerShell — see Development Environment for why a second implementation exists) both assume the operator has already written a stock Debian netinst ISO to a USB stick with **Rufus in "ISO Image mode"** (not "DD Image mode" — DD mode dd's the raw ISO9660 image, which is read-only and can't be written to at all; both scripts check the filesystem type first and abort with that exact instruction if it's not FAT32/exFAT). "ISO Image mode" extracts the ISO's files onto a real, writable filesystem, so both scripts then do plain file operations: copy `preseeds/classpad/preseed.cfg` (password token substituted from `CLASSPAD_USER_PASSWORD_CRYPTED`, never committed for real — see that file's header) to the drive root, copy a full `git archive` of this repo to `classpad-src/` on the drive (pinned to whatever commit is checked out when the script runs), and rewrite `isolinux/isolinux.cfg` (BIOS) and `boot/grub/grub.cfg` (UEFI) with a single self-contained auto-booting menu entry pointing at the preseed — rewritten wholesale rather than patched in place (Debian's default isolinux config is a multi-file `include` chain not worth reasoning about for a single-purpose provisioning stick), with the original backed up as `.orig` the first time so re-running either script is safe. Both scripts are re-runnable and idempotent for this reason.
- **The old "boots into a terminal login instead of the kiosk" bug from the v1 preseed had two independent causes, both now fixed in `install.sh` — but the second one is the one that actually mattered.**
  1. The preseed's `tasksel` is deliberately minimal (`standard`, no desktop task — `install.sh` installs every classpad package itself), so nothing ever told the freshly-installed base system to boot to `graphical.target` instead of `multi-user.target` (text console). `install.sh`'s own `systemctl set-default graphical.target` step (see "Same day" note further down this file) fixes this — but on its own, this fix could never have been reached.
  2. **`install.sh` runs (via `late_command`, below) inside d-i's `in-target` chroot, where no systemd is actually alive as PID 1.** Under `set -euo pipefail`, `install.sh`'s pre-existing `systemctl enable --now unattended-upgrades.service` and `systemctl daemon-reload` / `systemctl enable --now classpad-plugin-deploy.timer` calls (the `--now`/`daemon-reload` parts specifically — those try to talk to a live systemd over D-Bus and fail outright in a chroot) would abort the whole script right there, at step 4 of 15 — well before ever reaching the `graphical.target` line near the end. Fixing `graphical.target` alone would not have fixed the reported symptom; this second cause is what actually explains "install.sh ran [without erroring visibly, because early terminal-login v1 provisioning had no captured log — see below] but the machine came up wrong." Fixed with `enable_and_start_if_live()` in `install.sh`, which always does the symlink-only `enable` (safe anywhere) but only does `start`/`daemon-reload` when `/run/systemd/system` exists (i.e., a real running systemd, per systemd.exec(5) — present on a normal post-boot invocation, absent under `in-target`).
- **`preseed/late_command` runs `install.sh` with `CLASSPAD_SKIP_REBOOT=1`, logged, with conditional cleanup.** `install.sh` reboots itself automatically at the very end (see "install.sh reboots automatically" note below) — a raw `reboot` inside the chroot would fire mid-sequence instead of after d-i's own finish-install step, so it's suppressed here. `install.sh`'s stdout/stderr is captured to `/target/var/log/classpad-install.log` (survives on the real disk regardless of outcome) and the staged `classpad-src/` is only deleted on success, with `late_command` re-raising `install.sh`'s own exit status so a genuine failure halts d-i with a visible error instead of silently rebooting into a half-provisioned machine. The v1 preseed did none of this — a failed `install.sh` there left nothing behind to explain what happened, which is part of why cause #2 above went undiagnosed the first time.
- **Bug found and fixed on real hardware (2026-08-09), first real boot of a v2-provisioned stick:** `install.sh` step 1/15's `apt-get update` failed outright (`E: The repository 'cdrom://[...] trixie Release' does not have a Release file.`, apt's exit 100), aborting the whole script under `set -e` before any further step ran. Root cause: `install.sh` runs from `late_command` at d-i's `07preseed` finish-install hook, but the target's `/etc/apt/sources.list` still carries the installer's own `deb cdrom:...` entry at that point — d-i's own cleanup of it (`finish-install.d/10apt-cdrom-setup`) runs at hook prefix `10`, strictly *after* `07preseed`. The real mirror sources were already configured and reachable (confirmed `Hit:` for trixie/trixie-security/trixie-updates in the same log), but apt-get treats any one failed source as fatal for the whole `update` regardless. Diagnosed via the target's own IP serving d-i's built-in log HTTP server (`http://<target-ip>/syslog` — live for as long as d-i is sitting at its post-finish-install menu, which is what `late_command`'s exit-status-reraising already produces on a genuine failure, exactly as designed) rather than the `/target/var/log/classpad-install.log` file itself, which isn't exposed by that server. Fixed by stripping any `^deb cdrom:` line from `/etc/apt/sources.list` immediately before step 1's `apt-get update` (`sed -i '/^deb cdrom:/Id' /etc/apt/sources.list`) — confirmed harmless/no-op against a sources.list that's already clean (a normal post-boot re-run, after real `apt-cdrom-setup` has already run). **Not yet re-verified end-to-end** — this fixes the diagnosed failure but a full successful install with this fix in place hasn't been observed on real hardware yet; do that before considering v2 provisioning trustworthy for a fleet rollout.
- **`late_command` also `chmod 755`s every `.sh`/`.py` under the staged `classpad-src/`** before running `install.sh` — the USB is FAT32, which has no Unix exec-bit concept, so scripts can land there with none set regardless of what they had in the repo. `install.sh` itself is invoked via `bash <path>` so that alone doesn't strictly need it, but its own children (`plugin-install.sh`, `recovery.sh`, ...) get exec'd directly later at runtime and would otherwise inherit whatever FAT32 handed them. Run directly against the `/target/...` path from d-i's own shell, not nested inside a nother `in-target bash -c "..."` — that path doesn't need a chroot and the extra nesting is a needless third layer of quoting.
- **The stick must be physically removed once the install starts, and both scripts print a warning saying so.** The preseed reformats whatever disk it boots against with no further confirmation (`partman/confirm_nooverwrite` is preseeded true) — a stick left inserted with USB ahead of the internal disk in boot order silently re-provisions the machine again on the post-install reboot, or any later one. `isolinux.cfg`/`grub.cfg` are written with `prompt 1` and a real (not near-instant) timeout (10s) rather than an inert near-zero one, giving a bench operator an actual window to notice and interrupt if the stick wasn't pulled — but that's a mitigation, not a substitute for removing it.
- **What's still real, predates and is independent of all of the above:** `scripts/install.sh` itself and `scripts/wifi-configure.sh` (the plain `nmcli` WPA2-Enterprise connection-add logic, driven by `CLASSPAD_WIFI_SSID`/`IDENTITY`/`PASSWORD`/`CA_CERT` env vars, called from `install.sh`'s own WiFi step).
- **WiFi provisioning is still getting a fresh design pass**, not yet started — the v1 interactive WiFi TUI did not come back in v2. The env-var-driven `wifi-configure.sh` path (SSID/creds passed to `install.sh` at install time, e.g. by whoever's imaging the machine) still works and is the only WiFi-configuration path in the repo right now.
- Tests: `tests/test_build_provisioning_usb.py` runs the bash script against a synthetic fake USB tree (fake `findmnt` on `PATH`, same idiom as `tests/test_wifi_configure.py`'s fake `nmcli`) — covers the DD-mode rejection, the missing-password/missing-isolinux/missing-or-ambiguous-vmlinuz failure paths, the password substitution (including a regression case for a crypt hash containing `/`, which broke an earlier version that used `/` as sed's own delimiter), both boot-config rewrites, and re-run idempotency. No equivalent automated test exists for the `.ps1` — this dev environment has no `pwsh` — so it was written to mirror the bash script's logic exactly and reviewed by hand instead; re-verify it against a real Windows/PowerShell host before relying on it for a real fleet rollout.
...
- **Imaging/provisioning strategy — decided (v2), first real-hardware boot attempted 2026-08-09, not yet a clean end-to-end pass.** See "Provisioning / Imaging" above for `scripts/build-provisioning-usb.sh`/`.ps1` and the rebuilt `preseeds/classpad/preseed.cfg`. Unlike the reverted v1, this has been reasoned through carefully (FAT32-vs-ISO9660 filesystem detection, boot-config rewrite ordering, the two independent causes of v1's "boots to a text login" bug — `graphical.target` *and* `install.sh`'s `systemctl ... --now`/`daemon-reload` calls failing under `in-target`'s chroot — `CLASSPAD_SKIP_REBOOT`, and logged/conditional late_command cleanup) and has unit-test coverage for the bash script against a synthetic USB tree. The first real boot from a v2-provisioned stick surfaced and got a real bug fixed (the `deb cdrom:` apt-source-ordering failure — see the "Bug found and fixed on real hardware (2026-08-09)" bullet above), which is real signal the design's failure-visibility (late_command's exit-status reraising, the target's own log HTTP server) works as intended — but a full successful install with that fix in place hasn't been observed yet. Re-attempt the boot before trusting this for a real fleet rollout. WiFi provisioning specifically is still getting a fresh design pass; the only WiFi path in the repo right now is `scripts/wifi-configure.sh`'s env-var-driven `nmcli` call from `install.sh`.
...
├── preseeds/
│   └── classpad/
│       └── preseed.cfg         # Template — @@USER_PASSWORD_CRYPTED@@ substituted at USB-write time
├── scripts/
...
│   ├── wifi-configure.sh      # nmcli WPA2-Enterprise connection-add — called by install.sh's WiFi step
│   ├── build-provisioning-usb.sh    # Preps a Rufus-written USB stick for unattended install (bash)
│   └── build-provisioning-usb.ps1   # Same, for the Windows/WSL2 server dev machine (PowerShell)
└── tests/                     # pytest unit tests (run via python3-pytest, apt-installed)
    ...
    ├── test_build_provisioning_usb.py  # Fake findmnt + synthetic USB tree — bash script only, no pwsh here
...
- **`install.sh` reboots automatically at the end, added 2026-08-07.** Final step (`[15/15]`) — several earlier steps only take effect on a fresh boot (hostname change, GRUB/VT-switch config, the LightDM autologin drop-in), and leaving that to whoever ran the script (often non-technical — same "no admin regularly checking in" reasoning as the unattended-upgrades item above) risked a machine left half-applied indefinitely. `CLASSPAD_SKIP_REBOOT` (any non-empty value) opts out, for the test suite or anyone re-running `install.sh` by hand who wants to inspect the result before rebooting.
- **Same day: `install.sh` also runs `systemctl set-default graphical.target`** as part of the autologin step — a minimal, no-desktop-task base OS install leaves the system's default systemd target at `multi-user.target`, and installing the `lightdm` *package* alone only wires `lightdm.service` into `graphical.target`'s dependency tree, which is never reached unless something is actually booting to `graphical.target`. Cheap, offline (symlink-only), idempotent — safe to run unconditionally regardless of how the underlying OS was installed.
