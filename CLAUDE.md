...
**Preseed-based USB provisioning was added 2026-08-06 and removed again 2026-08-07 — currently an open question, not a decided design.** It briefly existed as `preseeds/classpad/preseed.cfg` plus a USB-prep script (first wrapping a vendored third-party ISO generator, then rewritten to inject a preseed/boot-entry/repo-copy directly onto an already-Rufus-written stick) and a first-boot interactive WiFi TUI (`wifi-setup-interactive.sh` + a WiFi answer-file USB for fleet rollout). All of it — the preseed file, both USB-prep script variants, the ISO-generator vendoring, the interactive WiFi setup service/drop-in, and the answer-file USB tooling — was pulled back out in full; none of those files exist in the repo anymore, and this section is left here only as a pointer for anyone wondering why `git log` shows them. Golden-image/Clonezilla/`dd` imaging (TODO.md's Phase 16's original sketch) was never built either, so imaging/provisioning strategy is back to genuinely undecided — see TODO.md Phase 16 and `pre-build-decisions.md` §1.
- **What's still real:** `scripts/install.sh` itself (see "App Launching"/"Provisioning" details elsewhere in this doc for its own steps) and `scripts/wifi-configure.sh` (the plain `nmcli` WPA2-Enterprise connection-add logic, driven by `CLASSPAD_WIFI_SSID`/`IDENTITY`/`PASSWORD`/`CA_CERT` env vars, called from `install.sh`'s own WiFi step) — both predate and are independent of the removed provisioning work above.
- **WiFi provisioning is getting a fresh design pass**, not yet started. The env-var-driven `wifi-configure.sh` path (SSID/creds passed to `install.sh` at install time, e.g. by whoever's imaging the machine) still works and is the only WiFi-configuration path in the repo right now.
...
- **Imaging/provisioning strategy is undecided again.** A preseed-based USB approach was tried, built, and then removed in full 2026-08-06/07 — see "Provisioning / Imaging" above. WiFi provisioning specifically is getting a fresh design pass; the only WiFi path in the repo right now is `scripts/wifi-configure.sh`'s env-var-driven `nmcli` call from `install.sh`.
